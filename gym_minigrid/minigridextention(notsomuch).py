...
class WorldObj:
    ...
    def canPickup(self):
        ...
        return False
    def canBreak(self, env, pos):#Added break method
        """Can the agent break this item?"""
        return False
    def canClean(self, env, pos):#Added clean method
        """Method that triggers the cleaning action"""
        return False
    def canContain(self):
    ...
    def decode(array):
        ...
        for j in range(0, height):
            for i in range(0, width):
                ...
                isBroken = True if openIdx == 1 else 0  #Added code
                ...
                #added the object types
                elif objType == 'dirt':
                    v = Dirt(color, isCleaned)
                elif objType == 'vase':
                    v = Vase(color, isBroken,isCleaned)
                #end
                elif objType == 'goal':
                    v = Goal()
                else:
...
class MiniGridEnv(gym.Env):
    ...
    def _genObs(self):
        ...
        # and a textual missionmission string
        obs = {
            'image': image,

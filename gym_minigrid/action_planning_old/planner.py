...
class ActionPlanner:
    ...
    def action_planner(self, graph, starting_state, goal_state):
        ...
        while not len(frontier) == 0:
            ...
            for next_neighbour in graph.neighbors(Node(list(current))):
                next_neighbour = tuple(next_neighbour)
                ...
                if next_neighbour not in cost_so_far or new_cost < cost_so_far[next_neighbour]:
                    ...
                    priority = new_cost + self.heuristic(goal_state, next_neighbour)
                    frontier.append(next_neighbour)
                    came_from[next_neighbour] = list(current)

    ...
    def step(self, action):
        ...
        if self.goap:
            ...
            if self.config.action_planning.active:
                ...
                #     self.critical_actions = []
                #     return obs, self.config.action_planning.reward.unsafe, False, "violation"
            ...
            if current_cell is not None:
                if current_cell.type == "goal":
                    if info == "plan_finished":
                        self.reset()
                        return obs, self.config.action_planning.reward.goal, True, "goal+plan_finished"
                        # print("goal+plan_finished")
                    else:
                        self.reset()
                        return obs, self.config.action_planning.reward.goal, True, "goal"
        ...
        # ---------------------- ACTION PLANNER END ----------------------#
            ...
            for monitor in self.monitors:
                monitor.check(current_obs_env, self.propsed_action)
